...
public class PlayerHandler : MonoBehaviour
{
    ...
    {
        return rb.velocity.y < -1 && !IsGrounded();
    }
    ...
    #region PLATAFORM STUFF (NOT BEING USED)
    Transform plataformParent;
    public void AssignNewPlataform(Transform newParent)
    {
        plataformParent = newParent;
        transform.parent = plataformParent;
        
    }

    void HandlePlataformLogic()
    {
        if (!IsGrounded())
        {
            //we dont have plataform if we are not grounded.
            AssignNewPlataform(null);
        }
    }
    #endregion
